...
if ! [[ -f "Vault%20Hunters%20Third%20Edition-3.21.6-server-files.zip" ]]; then
  rm -fr config mods defaultconfigs scripts forge-*.jar start.sh *.zip
	curl -Lo 'Vault%20Hunters%20Third%20Edition-3.21.6-server-files.zip' 'https://edge.forgecdn.net/files/8509/167/Vault%20Hunters%20Third%20Edition-3.21.6-server-files.zip' && unzip -u -o 'Vault%20Hunters%20Third%20Edition-3.21.6-server-files.zip' -d /data
